...
static SYM *mk_text_labeled(int lbl, const char *content_with_quotes)
{
    ...
    if (exist)
    {
        /* If existing is empty placeholder and new is not, update it */
        if (strcmp(exist->name, "\"\"") == 0 && strcmp(content_with_quotes, "\"\"") != 0)
        {
            free(exist->name);
            exist->name = strdup(content_with_quotes);
        }
        return exist;
    }
    SYM *sym = mk_sym();
...
/* 内存优化：STATIC 寄存器缓存 */
static int tp_valid = 0; /* 标记 R_TP 是否已加载 STATIC */
...
{
    /* 内存优化：不再在分支前回写所有寄存器，让寄存器状态跨越跳转继续生效 */
    if (a != NULL)
    {
        int r;
        for (r = R_GEN; r < R_NUM; r++)
...
void asm_code(TAC *c)
{
    ...
    case TAC_IFZ:
        ...
        return;
    case TAC_LABEL:
        /* 内存优化：仅当通过跳转到达该标签时，才执行回写与清理
           - 跳转到达：需要将上个基本块中被修改的变量落盘
